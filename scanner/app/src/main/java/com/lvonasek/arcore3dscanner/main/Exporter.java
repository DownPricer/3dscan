...
public class Exporter
{
  ...
  public static File export(File file, String filename) {
    ...
    if (file.getAbsolutePath().endsWith(Exporter.EXT_OBJ)) {
      ...
      for (String s : Exporter.getObjResources(file.getAbsoluteFile())) {
        File source = new File(file.getParentFile(), s);
        File target = new File(AbstractActivity.getPath(false), s);
        if (!moveFile(source, target)) {
          Log.e(AbstractActivity.TAG, "[TEXTURE][ERROR] Failed to export resource " + source);
          success = false;
        } else {
          Log.d(AbstractActivity.TAG, "File " + s + " saved");
        }
      }
      if (moveFile(file, file2save)) {
        Log.d(AbstractActivity.TAG, "Obj file " + file2save + " saved.");
      } else {
        Log.e(AbstractActivity.TAG, "[TEXTURE][ERROR] Failed to export OBJ file " + file);
        success = false;
      }
      if (!success) {
        return null;
  ...
  private static boolean moveFile(File source, File target) {
    ...
    if (!target.exists() || (target.length() == 0 && source.length() > 0)) {
      return false;
    }
