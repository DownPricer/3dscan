...
public abstract class AbstractActivity extends Activity {
  ...
  private static final String LEGACY_MODEL_DIRECTORY = "/3D Live Scanner/";
  private static final String NEW_MODEL_DIRECTORY = "/SiteReady Scan/";
  ...
  public static String getPath(boolean migrate) {
    ...
    String legacydir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOCUMENTS).getPath() + LEGACY_MODEL_DIRECTORY;
  ...
  public static boolean hasFilesToMigrate(Context context) {
    ...
    if (new File(legacydir).exists()) {
      File[] files = new File(legacydir).listFiles();
      if ((files != null) && (files.length > 0)) {
        return true;
      }
    }
